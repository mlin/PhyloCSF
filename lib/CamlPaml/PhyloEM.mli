...
(** compute the gradient of the ell with respect to the variables in the model's rate matrix p14n, based on their symbolic expressions *)
val d_ell_dQ_dx : PhyloModel.P14n.instance -> sufficient_statistics -> float array

(** numerically estimate the same *)
val nd_ell_dQ_dx : PhyloModel.P14n.instance -> sufficient_statistics -> float array

(** compute the derivative with respect to one specific rate matrix variable only *)
val d_ell_dQ_dxi : PhyloModel.P14n.instance -> sufficient_statistics -> int -> float

(** compute the gradient with respect to the variables in the model's p14n for branch lengths, based on their symbolic expressions*)
